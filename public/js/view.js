(() => {
  ...
  burnBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        burnBtn.textContent = 'Reveal & Destroy Secret';
        return;
      }
      ...
      if (data.burned || data.views_remaining === 0) {
        ...
        burnStatusDesc.textContent = 'This secret has now been destroyed and cannot be retrieved again.';
      } else {
  ...
  if (downloadSecretBtn) {
    downloadSecretBtn.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = `secret-${secretId}.txt`;
