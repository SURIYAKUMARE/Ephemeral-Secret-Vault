/**
 * Ephemeral Secret Vault — Gmail Platform Adapter
 */
...
(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    ...
    root.GmailAdapter = factory(root.PlatformAdapter);
  }
})(typeof globalThis !== 'undefined' ? globalThis : this, function (PlatformAdapter) {
  'use strict';

  class GmailAdapter extends PlatformAdapter {
    constructor() {
      super('Gmail');
    }
    ...
    getComposer() {
      const selectors = [
        ...
        'table.cf div[contenteditable="true"]'
      ];
    ...
    injectToolbarButton(onClick) {
      if (document.getElementById('ephemeral-gmail-btn')) return;

      // Find Gmail compose bottom bar (near Send button)
      const toolbar = document.querySelector('tr.btC, td.gU.Up, div[aria-label*="Formatting options"]')?.parentElement;
      if (!toolbar) return;

      const btn = document.createElement('button');
      btn.id = 'ephemeral-gmail-btn';
      btn.type = 'button';
      btn.title = 'Protect sensitive text with Ephemeral Vault';
      btn.innerHTML = '🔐 Protect Secret';
      btn.style.cssText = 'background: rgba(99, 102, 241, 0.15); border: 1px solid rgba(99, 102, 241, 0.4); color: #818cf8; font-weight: 600; font-size: 12px; padding: 6px 10px; border-radius: 6px; cursor: pointer; margin-left: 8px; display: inline-flex; align-items: center; gap: 4px;';
